...
export default function DropdownConditional(props){
    ...
    console.log(console.log(props.selectedPet))
    const mid = ()=>{
        const elements = [];
      ...
      const menu = (
        <Menu style={{ marginLeft: -23, borderBottomLeftRadius: 30, borderBottomRightRadius: 30,marginTop:10}}>
          <Menu.Item className="MenuItem" style={{paddingLeft:30,paddingRight:30,marginTop:-5}}>
            <a target="_blank" rel="noopener noreferrer" onClick={() => props.setProperty(props.items[props.selectedPet][props.selectedPet][0])}>
              {props.blankValue}
            </a>
          </Menu.Item>
          {mid()}
          <Menu.Item className="MenuItem" style={{paddingLeft:30,paddingRight:30,borderBottomLeftRadius:30,borderBottomRightRadius:30,marginBottom:-5}}>
            <a target="_blank" rel="noopener noreferrer" onClick={() => props.setProperty(props.items[props.selectedPet][props.items[props.selectedPet].length-1])}>
